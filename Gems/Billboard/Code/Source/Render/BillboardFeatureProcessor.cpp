...
#include "BillboardFeatureProcessor.h"
...
namespace Billboard {
    void BillboardFeatureProcessor::Reflect(AZ::ReflectContext *context) {
        if (auto *serializeContext = azrtti_cast<AZ::SerializeContext *>(context)) {
            serializeContext
                    ->Class<BillboardFeatureProcessor, FeatureProcessor>();
        }
    }

    void BillboardFeatureProcessor::Activate() {
        AZ_Printf("BillboardFeatureProcessor", "BillboardFeatureProcessor Activated");
        ...
        if (!m_shader) {
            ...
            AZ_Error("BillboardFeatureProcessor", false, "Failed to load required stars shader.");
            return;
        }
        ...
        AZ_Error("BillboardFeatureProcessor", drawSrgLayout,
                 "Failed to get the draw shader resource group layout for the Billboard shader.");
        ...
        AZ_Assert(viewportContextInterface,
                  "BillboardFeatureProcessor requires the ViewportContextRequestsInterface.");
        auto viewportContext = viewportContextInterface->GetViewportContextByScene(GetParentScene());
        AZ_Assert(viewportContext, "BillboardFeatureProcessor requires a valid ViewportContext.");
    ...
    AZStd::vector<float> BillboardFeatureProcessor::ConvertToBuffer(const AZStd::vector<CloudVertex> &cloudVertexData){
    ...
    void BillboardFeatureProcessor::SetCloud(const AZStd::vector<CloudVertex> &cloudVertexData) {
    ...
    void BillboardFeatureProcessor::UpdateDrawPacket() {
        // print if  things
        printf("BillboardFeatureProcessor UpdateDrawPacket\n");
        // variables
    ...
    void BillboardFeatureProcessor::OnAssetReloaded(AZ::Data::Asset<AZ::Data::AssetData> asset) {
        AZ_Printf("BillboardFeatureProcessor", "BillboardFeatureProcessor OnAssetReloaded");
        UpdateDrawPacket();
        UpdateBackgroundClearColor();
    }

    void BillboardFeatureProcessor::Deactivate() {
        AZ_Printf("BillboardFeatureProcessor", "BillboardFeatureProcessor Deactivated");
        AZ::Data::AssetBus::Handler::BusDisconnect(m_shader->GetAssetId());
        AZ::RPI::ViewportContextIdNotificationBus::Handler::BusDisconnect();
    }

    void BillboardFeatureProcessor::Simulate([[maybe_unused]] const FeatureProcessor::SimulatePacket &packet) {
    ...
    void BillboardFeatureProcessor::Render([[maybe_unused]] const FeatureProcessor::RenderPacket &packet) {
    ...
    void BillboardFeatureProcessor::UpdateBackgroundClearColor()
    {
    ...
    AZ::RHI::ConstPtr<AZ::RHI::DrawPacket> BillboardFeatureProcessor::BuildDrawPacket(
    ...
    void BillboardFeatureProcessor::OnRenderPipelineChanged([[maybe_unused]] AZ::RPI::RenderPipeline *renderPipeline,
    ...
    void BillboardFeatureProcessor::UpdateShaderConstants() {
        if (m_drawSrg) {
            AZ_Printf("BillboardFeatureProcessor", "BillboardFeatureProcessor SetPointSize");
...
//            Texture2D<float4> m_inputTexture; // A texture generated by a RenderJoy pipeline.
    ...
    void BillboardFeatureProcessor::SetTransform(const AZ::Transform &transform) {
        m_updateShaderConstants = true;
        m_transform = transform;
    }

    void BillboardFeatureProcessor::SetPointSize(float pointSize) {
        m_updateShaderConstants = true;
