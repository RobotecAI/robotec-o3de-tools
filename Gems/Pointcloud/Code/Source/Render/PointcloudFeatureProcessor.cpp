...
namespace Pointcloud {
   ...
   void PointcloudFeatureProcessor::Activate() {
       ...
       bufferPoolDesc->m_budgetInBytes = 2u* 1024u * 1024u * 1024u; // 1GB
       bufferPoolDesc->m_largestPooledAllocationSizeInBytes = 1024u * 1024u * 1024u; // 1GB
   ...
   void PointcloudFeatureProcessor::SetCloud(const AZStd::vector<PointcloudAsset::CloudVertex> &cloudVertexData) {
       const uint32_t elementSize = sizeof(cloudVertexData);
       const uint32_t elementCount = static_cast<uint32_t>(cloudVertexData.size()*0.1);
       ...
       AZ::RPI::BufferAssetCreator bufferCreator;
       bufferCreator.Begin(AZ::Uuid::CreateRandom());
       bufferCreator.SetPoolAsset(m_resourcePoolAsset);

       AZ::RHI::BufferDescriptor newBufferDescriptor;
       newBufferDescriptor.m_bindFlags = AZ::RHI::BufferBindFlags::InputAssembly;
       newBufferDescriptor.m_byteCount = bufferSize;
       bufferCreator.SetBuffer(m_starsMeshData.data(), newBufferDescriptor.m_byteCount, newBufferDescriptor);
       bufferCreator.SetBufferViewDescriptor(AZ::RHI::BufferViewDescriptor::CreateStructured(
           0, elementCount, elementSize));
       bufferCreator.End(m_cloudVertexBufferAsset);
        m_cloudVertexBuffer = AZ::RPI::Buffer::FindOrCreate(m_cloudVertexBufferAsset);
