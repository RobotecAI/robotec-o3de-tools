...
{
    AZ_COMPONENT_IMPL(SensorDebugEditorSystemComponent, "SensorDebugEditorSystemComponent",
        SensorDebugEditorSystemComponentTypeId, BaseSystemComponent);

    void SensorDebugEditorSystemComponent::Reflect(AZ::ReflectContext* context)
    {
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serializeContext->Class<SensorDebugEditorSystemComponent, SensorDebugSystemComponent>()
                ->Version(0);
        }
    }
