...
namespace Billboard
{
    ...
    class BillboardFeatureProcessor
        : public BillboardFeatureProcessorInterface
        , protected AZ::RPI::ViewportContextIdNotificationBus::Handler
        , protected AZ::Data::AssetBus::Handler
    {
    public:
        AZ_RTTI(BillboardFeatureProcessor, "{B6EF8776-F7F9-432B-8BD9-D43869FFFC11}", BillboardFeatureProcessorInterface);
        AZ_CLASS_ALLOCATOR(BillboardFeatureProcessor, AZ::SystemAllocator)

        static void Reflect(AZ::ReflectContext* context);

        BillboardFeatureProcessor() = default;
        virtual ~BillboardFeatureProcessor() = default;

        // BillboardFeatureProcessorInterface
